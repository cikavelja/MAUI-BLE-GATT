...
namespace MauiBleApp2.Core.Services.Bluetooth
{
    ...
    public class DeviceScanService : IDeviceScanService
    {
        ...
        public async Task StartScanningAsync(int scanTimeout = 10000, CancellationToken cancellationToken = default)
        {
            try
            {
                Console.WriteLine("DeviceScanService: Starting BLE scan");
                await _bluetoothService.StartScanningAsync(scanTimeout, cancellationToken);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"DeviceScanService: Failed to start scanning: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task StopScanningAsync()
        {
            try
            {
                Console.WriteLine("DeviceScanService: Stopping BLE scan");
                await _bluetoothService.StopScanningAsync();
            }
            catch (Exception ex)
            {
                Console.WriteLine($"DeviceScanService: Failed to stop scanning: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<BleDeviceInfo?> ConnectToDeviceAsync(string deviceId, CancellationToken cancellationToken = default)
        {
            try
            {
                Console.WriteLine($"DeviceScanService: Connecting to device {deviceId}");
                return await _bluetoothService.ConnectToDeviceAsync(deviceId, cancellationToken);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"DeviceScanService: Failed to connect to device: {ex.Message}");
                throw;
            }
        }
        ...
        {
            Console.WriteLine($"DeviceScanService: Device discovered: {args.Device?.Name} ({args.Device?.Id})");
